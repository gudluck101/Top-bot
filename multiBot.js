...
async function send(bot) {
  ...
  for (let attempt = 1; attempt <= 10; attempt++) {
    try {
      ...
      const baseFeeStroops = Math.floor(baseFeePi * 1e7); // Convert Pi → stroops
...
// Trigger based on daily UTC time
setInterval(() => {
  const now = new Date();
  const nowMs =
